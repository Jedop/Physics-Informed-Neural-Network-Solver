...
class PINN(nn.Module):
    def __init__(self, n=1):
        ...
        self.E = nn.Parameter(torch.tensor([(n - 1) / 2]))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Solve for the nth eigenstate of the Harmonic Oscillator using a PINN."
    )
    parser.add_argument(
        "n",
        type=int,
        help="The principal quantum number (n) of the eigenstate to find (e.g., 1 and 2).",
    )
    ...
    psi_true = -harmonic_oscillator_wavefunction(n_state, x_np)
